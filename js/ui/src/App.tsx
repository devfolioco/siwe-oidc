  ...
  return (
    <div className={`flex p-5 gap-5 justify-start items-center rounded-lg ${isActive? 'border-2 border-solid border-blue-3':'border border-solid border-gray-3'}`}>
      {isCompleted ? 
        ...
        />
        :
        <div className={`flex justify-center items-center rounded-[32px] px-[13px] py-[8px] h-8 w-8 ${isActive ? 'bg-blue-3' : 'bg-gray-bg'}`}>
          <p className={`text-sm font-montserrat font-extrabold ${isActive ?  'text-white':'text-gray-6'}`}>
            {number}
          </p>
        </div>
      }
      <div className="flex flex-col gap-1 justify-start items-start">
        <p className="text-lg text-gray-8 font-bold">
          {header}
        </p>
...
function App() {
  ...
  const walletConnectStage: STEP_STAGE = activeStepNumber === 1 ? 'incomplete' : 'complete';
  const verifyAddressStage: STEP_STAGE = activeStepNumber <= 2 ? 'incomplete' : 'complete';
